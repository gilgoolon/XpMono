...
RelocationBlocksIterator::RelocationBlocksIterator(const void* module, bool& result):
	m_module(module),
...
{
	result = false;
	const IMAGE_DATA_DIRECTORY* const relocations_data_directory = Pe::get_data_directory(
		module,
...
bool RelocationBlocksIterator::has_next() const
{
	return m_current_offset < m_size;
}

RelocationBlocksIterator::Entry RelocationBlocksIterator::next()
{
	const Entry result = {
