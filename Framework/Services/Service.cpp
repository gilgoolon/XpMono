...
SC_HANDLE Service::open_service(const std::wstring& name)
{
	...
	static constexpr DWORD SERVICE_MANAGER_CONNECT_ACCESS = SC_MANAGER_CONNECT;

	const SC_HANDLE service_manager = OpenSCManagerW(
		LOCAL_SERVICE_MANAGER,
		DEFAULT_SERVICE_MANAGER_DB,
		SERVICE_MANAGER_CONNECT_ACCESS
	);
	if (service_manager == nullptr)
	{
		TRACE("OpenSCManagerW failed with error: ", GetLastError());
		throw WinApiException(ErrorCode::FAILED_SERVICE_OPEN_MANAGER);
	}

	static constexpr DWORD SERVICE_QUERY_STATUS_ACCESS = SERVICE_QUERY_STATUS;
	const SC_HANDLE service_handle = OpenServiceW(service_manager, name.c_str(), SERVICE_QUERY_STATUS_ACCESS);
...
SERVICE_STATUS_PROCESS Service::query_status() const
{
	...
	{
		TRACE("QueryServiceStatusEx failed with error: ", GetLastError());
